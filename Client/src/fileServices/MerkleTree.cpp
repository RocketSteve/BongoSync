...
void MerkleTree::buildTree(const std::string& directoryPath) {
    ...
    std::filesystem::path path = std::filesystem::path(Utility::getBongoDir()) / "tree.json";
    Utility::updateWorkspaceHash(getTreeHash());
    saveToFile(path);
}
    ...
    if (std::filesystem::is_directory(path)) {
        for (const auto& entry : std::filesystem::directory_iterator(path)) {
            ...
            if (std::filesystem::is_directory(entry.path())) {
                ...
                std::filesystem::path debug = std::filesystem::path(root->path) / entry.path();
